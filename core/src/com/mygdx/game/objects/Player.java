...
public class Player {
    ...
    public void WallControls(){
        if(Gdx.input.isKeyPressed(Input.Keys.RIGHT) && !leftWallCol){
            XVelocity = 0;
            x += 1;
        }
        if(Gdx.input.isKeyPressed(Input.Keys.LEFT) && !rightWallCol){
            XVelocity = 0;
            x -= 1;
        }
        if(Gdx.input.isKeyPressed(Input.Keys.LEFT) && !leftWallCol){
            facingRight = false;
            YVelocity = 0;
            falling = false;
        ...
        if(Gdx.input.isKeyPressed(Input.Keys.RIGHT) && !rightWallCol){
            facingRight = true;
            YVelocity = 0;
            falling = false;
            if(Gdx.input.isKeyPressed(Input.Keys.DOWN)){
        ...
        if(Gdx.input.isKeyPressed(Input.Keys.DOWN) && !ceilingCol){
            y = this.y - 1;
            YVelocity = -1;
        }
        else if(Gdx.input.isKeyPressed(Input.Keys.RIGHT) && !ceilingCol){
            crawlTimer += 1;
        }
        else if(Gdx.input.isKeyPressed(Input.Keys.LEFT) && !ceilingCol){
            crawlTimer += 1;
        }
        if(crawlTimer > 20){
            crawlTimer = 0;
    ...
    public void Move(){
        ...
        else if(Gdx.input.isKeyPressed(Input.Keys.RIGHT) && rightWallCol){
            XVelocity = xSpeed * Gdx.graphics.getDeltaTime();
            facingRight = true;
            runAnimTimer += 1;
        }
    ...
    public void BalloonJump(){
        ...
        else if(Gdx.input.isKeyJustPressed(Input.Keys.UP) && !balloon && falling){
            YVelocity = 80;
            balloon = true;
        }
        if(balloon){
